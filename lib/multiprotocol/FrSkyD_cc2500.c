...
static void __attribute__((unused)) frsky2way_init(uint8_t bind, radio_t *radio)
{
	FRSKY_init_cc2500(FRSKYD_cc2500_conf, radio);	
...
{//pachet[4] is telemetry user frame counter(hub)
	...
	{
		uint16_t value;
			value = convert_channel_frsky(i, radio);
		if(i < 4)
		{
...
uint16_t initFrSky_2way(radio_t *radio)
{
	...
	if(IS_BIND_IN_PROGRESS(radio->flags))
	{
		frsky2way_init(1, radio);
